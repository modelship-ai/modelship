...
- **Routing derived from Serve** — a head-node actor (`GatewayCoordinator`) computes each gateway's model → deployment table once a second from Ray Serve's application statuses and the gateway's effective config. A changed model keeps being served by its old deployment until the new one has a running replica; a deployment nothing uses is deleted 10 s later, and `mship deploy` waits for that. Nothing is stored, so a restarted gateway coordinator recomputes the tables
