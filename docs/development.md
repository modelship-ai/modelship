...
| `MSHIP_RAY_DASHBOARD_HOST` | `127.0.0.1` | Ray dashboard bind host, `start` only (`--ray-dashboard-host` flag). The dashboard always starts; this sets *where* it binds — `0.0.0.0` exposes it beyond the container (ShadowRay/CVE-2023-48022 exposure vector; only do this on a trusted/private network). |
...
| `MSHIP_RAY_DASHBOARD_PORT` | `8265` | **Optional override:** Ray dashboard port, `start` only (`--ray-dashboard-port` flag). Only needed to run multiple modelship heads on one host under `--network=host`, where Ray's fixed default would otherwise collide between them. |
