...
def _field_ref(container: dict, name: str) -> str | None:
    ...
    return entry["valueFrom"]["resourceFieldRef"]["resource"] if entry and "valueFrom" in entry else None
...
def main() -> int:
    cluster = _cluster(
        {
            "workerGroups": [
                _group("req", resources={"requests": {"cpu": "4"}, "limits": {"memory": "8Gi"}}),
                _group("lim", resources={"limits": {"cpu": "2"}}, env=[{"name": "MSHIP_NODE_MEMORY", "value": "4Gi"}]),
                _group("own", resources={"limits": {"cpu": "2"}}, env=[{"name": "MSHIP_NODE_NUM_CPUS", "value": "1"}]),
                _group("bare"),
            ]
        }
    )
    ...
    assert _field_ref(req, "MSHIP_NODE_MEMORY") == "limits.memory"
